...
// Fix default marker icons — same as UserDashboard
...
// ── Icons — copied exactly from UserDashboard ─────────────────────────────────
...
// ── RouteDisplay — copied exactly from UserDashboard ─────────────────────────
...
const DriverDashboard = () => {
  const [ambulance, setAmbulance] = useState(null);
  const [emergency, setEmergency] = useState(null);
  const [ambulanceLocation, setAmbulanceLocation] = useState(null); // same as UserDashboard
  ...
  const loadData = async () => {
    try {
      ...
      // ambRes.data[0] is the active emergency object for this driver
      const ambRes = await ambulanceAPI.getByDriver(user.username);
      ...
      // Fetch graph node coordinates — same as UserDashboard
      ...
      // Fetch ambulance location using assignedAmbulanceId — same pattern as UserDashboard
  ...
  // Mirrors UserDashboard's fetchAmbulanceLocation exactly
  ...
  const setupSocket = () => {
    ...
    socketService.on("ambulanceLocationUpdate", (data) => {
      // Mirror UserDashboard socket handler for live ambulance updates
      ...
      <div className="dashboard-grid">
        ...
        <div className="panel ambulance-panel">
          ...
          <div className="info-grid">
            ...
            <div className="info-item">
              <label>Driver Name</label>
              <span className="value">{ambulanceLocation?.driver?.name || ambulance?.driver?.name || user.username}</span>
            </div>
            <div className="info-item">
              <label>Contact</label>
       ...
       <div className="live-map-container" style={{  margin: "40px" }}>
                ...
                >
                  ...
                  {/* Patient Location — same as UserDashboard */}
                  ...
                  {/* Ambulance Location — same as UserDashboard, uses ambulanceLocation.currentLocation */}
                  ...
                  {/* Route — same condition as UserDashboard */}
                  {ambulanceLocation &&
